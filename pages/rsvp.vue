...
<template>
  <div class="min-h-screen bg-warm-cream text-deep-espresso select-text flex flex-col" :class="isFormActive ? 'justify-center' : 'justify-between'">
    ...
    <main class="flex-1 flex items-center justify-center p-6" :class="isFormActive ? 'pt-6 pb-6' : 'pt-32 pb-16'">
      <div class="w-full max-w-2xl">
        <div v-if="!mounted" class="min-h-[300px] flex items-center justify-center">
          <div class="text-sm font-semibold tracking-wider uppercase font-display-cinzel animate-pulse">Loading RSVP details...</div>
        </div>

        <template v-else>
          <!-- 1. INVALID DIRECT LINK BLOCKER -->
          <div v-if="invalidLinkError && !existingRSVP" class="linen-card p-8 rounded-2xl border border-amber-gold/20 shadow-lg text-center space-y-5 animate-fade-in">
          ...
          <div v-if="groupFullError" class="linen-card p-8 rounded-2xl border border-red-500/20 shadow-lg text-center space-y-5 animate-fade-in">
            <div class="text-4xl">⚠️</div>
          ...
          <div v-if="existingRSVP && !isEditing && !groupFullError" class="linen-card p-6 sm:p-10 rounded-2xl border border-amber-gold/15 shadow-md space-y-8 animate-fade-in">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-amber-gold/10 pb-6">
              <div>
                <span class="text-[10px] uppercase tracking-wider font-bold text-amber-gold block mb-1">
                  RSVP Status
                </span>
                <div class="flex items-center gap-2.5">
                  <span class="w-3.5 h-3.5 rounded-full" :class="existingRSVP.attending ? 'bg-emerald-500' : 'bg-red-400 animate-pulse'" />
                  <h3 class="font-heading text-2xl font-bold text-deep-espresso">
              ...
              <div class="text-left sm:text-right text-xs text-deep-espresso/60 font-body">
                Submitted: {{ new Date(existingRSVP.submittedAt).toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" }) }}
              </div>
            </div>
            ...
            <div v-if="existingRSVP.dietaryNotes || existingRSVP.message" class="border-t border-amber-gold/10 pt-6 space-y-4 text-sm font-body">
              ...
              <div v-if="existingRSVP.message">
                ...
                <p class="text-deep-espresso/80 whitespace-pre-line bg-soft-pearl/50 p-4 rounded-xl border border-amber-gold/10">
          ...
          <!-- 4. ACTIVE FORM -->
          <form v-if="isFormActive" @submit.prevent="handleSubmit" class="linen-card w-full p-8 sm:p-12 rounded-3xl border border-amber-gold/15 shadow-2xl relative overflow-hidden select-text animate-fade-in">
            <!-- Progress Line -->
            ...
            <!-- Branding Context -->
            <div class="flex justify-between items-center mb-10 text-xs font-bold uppercase tracking-widest text-deep-espresso/45">
              <span>#TheSweetUnion</span>
              <span v-if="isEditing" class="text-deep-terracotta">Editing Mode</span>
            ...
            <div v-if="currentStep === 3" class="space-y-8 animate-fade-in">
              ...
              <div class="space-y-5 pt-2">
                <div class="space-y-1">
                  <label class="text-xs font-bold uppercase tracking-wider text-deep-espresso/60">Full Name (Required)</label>
                  <input
                    type="text"
                    required
                    v-model="leadName"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div class="space-y-1">
                    <label class="text-xs font-bold uppercase tracking-wider text-deep-espresso/60">Email Address (Required)</label>
                    <input
                      type="email"
                      required
                      v-model="leadEmail"
                ...
                <div class="space-y-3 bg-soft-pearl/60 p-4 rounded-xl border border-amber-gold/15">
                  <h4 class="text-xs font-bold uppercase tracking-wider text-amber-gold mb-1">Which events are you joining?</h4>
                ...
                <div class="space-y-4 pt-2">
                  ...
                  <div v-if="hasSpouse" class="space-y-1 pl-7 animate-fade-in">
                    <label class="text-xs font-bold uppercase tracking-wider text-deep-espresso/60 block">Spouse / Partner Name (Required)</label>
                    <input
                      type="text"
                      required
                      v-model="spouseName"
                ...
                <div class="space-y-1">
                  <label class="text-xs font-bold uppercase tracking-wider text-deep-espresso/60">Dietary restrictions (Optional)</label>
                  <input
                    type="text"
              ...
              <div class="pt-4 flex items-center justify-end">
                <button
                  ...
                  :class="(!leadName || !leadEmail || (hasSpouse && !spouseName.trim()) || (!events.ceremony && !events.reception))
            ...
            <div class="border-t border-amber-gold/10 mt-8 pt-4 flex justify-between items-center text-xs font-bold uppercase tracking-wider">
              <button
                v-if="currentStep > 2"
                type="button"
    ...
    <!-- SUCCESS MODALS -->
    <div v-if="successModal" class="fixed inset-0 bg-black/70 z-[9999] flex items-center justify-center p-4 animate-fade-in animate-duration-200">
      <div class="linen-card w-full max-w-md p-8 rounded-2xl border border-amber-gold/20 shadow-2xl text-center relative animate-scale-up animate-duration-200">
        ...
        <h3 class="font-heading text-2xl font-bold text-deep-espresso mb-2">
          {{ successModal === "cancel" ? "RSVP Removed" : successModal === "edit" ? "RSVP Changes Saved!" : "Thank You So Much!" }}
        </h3>

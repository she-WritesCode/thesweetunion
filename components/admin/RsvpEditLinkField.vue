...
<template>
  <div class="rsvp-link-field">
    <p v-if="!editToken" class="rsvp-link-field__empty">Save the record first to generate the Edit link.</p>
    ...
    <template v-else>
      <div class="rsvp-link-field__row">
        ...
        <button
          ...
          @click="copyLink"
          :aria-label="copied ? 'Copied!' : 'Copy Edit link'"
        >
      ...
      <p class="rsvp-link-field__hint">
        {{ field?.admin?.description ?? "Share this link with your guest so they can edit their RSVP." }}
      </p>
    </template>
  </div>
</template>

...
const asoebiSent = computed(() => {
  const val = record.value.asoebiSent;
  return val === true || val === "true" || val === 1 || val === "1";
});
const asoebiSentAt = computed(() => (record.value.asoebiSentAt as string) ?? "");

const sentBadgeLabel = computed(() => {
  if (!asoebiSent.value) return null;
  const time = asoebiSentAt.value
    ? new Date(asoebiSentAt.value).toLocaleDateString("en-GB", {
        day: "numeric",
        month: "short",
        hour: "2-digit",
        minute: "2-digit",
...
async function sendReminder() {
  ...
  try {
    ...
    if (id) {
      ...
      if (dyrected?.client) {
        await dyrected.client.collection("rsvp_records").update(id, {
          asoebiSent: true,
          asoebiSentAt: now,
        });
      } else {
        await $fetch(`/api/dyrected/api/collections/rsvp_records/${id}`, {
          method: "PATCH",
          headers: adminAuthHeaders(),
          body: {
            asoebiSent: true,
      ...
      if (fetchedDoc.value) {
        fetchedDoc.value.asoebiSent = true;
        fetchedDoc.value.asoebiSentAt = now;
      }
      if (props.onChange) {
        props.onChange({ asoebiSent: true, asoebiSentAt: now });
      }
    }
  } catch (err: any) {
    ...
    <template v-else>
      ...
      <div class="row">
        <button type="button" class="btn-wa" :disabled="loading || !customMessage.trim()" @click="sendReminder">
          ...
          {{ asoebiSent ? "Resend WhatsApp Reminder" : "Send WhatsApp Reminder" }}
        </button>
        <span class="sent-badge" v-if="sentBadgeLabel">✓ {{ sentBadgeLabel }}</span>

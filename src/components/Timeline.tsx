    ...
    <div className="relative border-l border-amber-gold/30 ml-4 md:ml-1/2 md:border-l-0 py-8 select-text">
      ...
      {items.map((item, index) => {
        ...
        return (
          <div
            key={item.key}
            className={`relative mb-16 last:mb-0 md:flex md:w-full items-center ${isEven ? "md:justify-start" : "md:justify-end"
              }`}
          ...
          >
            ...
            <div
              className={`absolute left-[-21px] md:left-1/2 md:ml-[-10px] w-5 h-5 rounded-full border-2 border-amber-gold bg-warm-cream flex items-center justify-center transition-all duration-500 z-20 ${isHovered ? "scale-125 border-deep-terracotta bg-linen-white" : ""
                }`}
            >
              <div
                className={`w-2 h-2 rounded-full bg-amber-gold transition-colors duration-500 ${isHovered ? "bg-deep-terracotta" : ""
                  }`}
              />
            </div>
            ...
            <div
              className={`w-full pl-6 md:pl-0 md:w-[45%] transition-all duration-500 ${isEven ? "md:pr-12 text-left md:text-right" : "md:pl-12 text-left"
            ...
            >
              <div className="linen-card overflow-hidden rounded-2xl border border-amber-gold/20 transition-all duration-500 hover:shadow-xl hover:border-amber-gold/45 bg-linen-white">
                ...
                <div className="relative w-full aspect-4/3 overflow-hidden bg-deep-espresso/5 border-b border-amber-gold/10">
                  ...
                  <div className="absolute inset-0 bg-linear-to-t from-deep-espresso/30 to-transparent pointer-events-none mix-blend-multiply" />
                </div>

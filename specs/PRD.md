...
Each RSVP group has its own **unique registration link** generated by the admin (e.g. `thesweetunion.com/rsvp?group=church-friends`). Guests are invited via personalised invitation cards (sent on WhatsApp and/or email) that contain their group's specific link. When a guest opens their link, the group is pre-selected and they proceed directly to the form — they never see a list of groups, and they cannot access another group's link.

Groups are an internal crowd management tool. From the guest's perspective, they simply received a link on their invitation. Group names only appear in the admin dashboard.
...
Full RSVP link format: `[domain]/rsvp?group=[slug]`

- The `slug` is the human-readable group identifier passed as a query parameter.
...
RSVPRecord {
  ...
  lead_email:      string        // unique site-wide — system prevents duplicate email across all groups
  lead_phone:      string (optional)
  has_spouse:      boolean       // true if attending with spouse/partner (default: false)
  spouse_name:     string        // full name of spouse (required if has_spouse = true)
...
> **Seat count rule:** Each RSVP counts as 1 seat (lead alone) or 2 seats (lead + spouse). `confirmed_count` is incremented by 1 or 2 accordingly.

> **Adults only, no general plus-ones:** Guests may only bring their spouse or life partner — not friends, colleagues, or other plus-ones. This is clearly communicated on the RSVP form.
...
- System validates the `slug` query parameter. If invalid or deactivated: shows a friendly error — _"This link is no longer active. Please contact the couple directly."_
...
**Step 3 — Spouse Details & Message**

- A single toggle: "Are you attending with your spouse / partner?"
  - If **Yes**: guest enters their spouse's full name (required). The form notes this is an adults-only event and the plus-one must be a spouse or life partner.
  - If **No**: proceeds as a solo attendee.
- System checks: `(confirmed_count + seats) ≤ max_capacity`, where `seats` = 1 (solo) or 2 (with spouse).
  - If no capacity: inline error — _"Sorry, there are no more spots available. Please contact the couple directly."_
- Dietary notes: optional text field covering the guest and their spouse if applicable.
- Optional message to the couple.

**Step 4 — Review & Submit**

- Summary card: Lead name, spouse name (if applicable), attending status, selected events.
- Guest clicks **Submit RSVP**.
- System atomically increments `confirmed_count` by 1 or 2 (attending only, based on `has_spouse`).
...
- Changes to `has_spouse` must recheck group capacity (solo = 1 seat, with spouse = 2 seats).
- If changing from Yes → No: `confirmed_count` is decremented by original seat count (1 or 2).
...
| RSVP submitted             | Guest          | Attending status, spouse name (if applicable), venue details, edit link |
| RSVP updated               | Guest          | Updated summary                                                         |
| New RSVP                   | Admin (couple) | Guest name, group, seat count (1 or 2), optional digest                 |
...
```sql
-- Atomic increment with guard
-- $seats = 1 (solo attendee) or 2 (attendee + spouse)
UPDATE rsvp_groups
SET confirmed_count = confirmed_count + $seats
WHERE id = $group_id
  AND (confirmed_count + $seats) <= max_capacity
RETURNING *;
...
| RSVP group names, capacity limits                                               | Week 2      |
...
| 2   | Should RSVP groups be listed publicly? | No. Groups are internal only. Guests arrive via their unique link and never see a group name or list. Groups are used for crowd management. |
...
| 11  | RSVP URL format                        | Group links use a query parameter format: `[domain]/rsvp?group=[slug]`. No token-based path authentication. Admin copies these links from the dashboard.        |
| 12  | Guest party size — spouse only         | Each guest may bring only their spouse or life partner as a plus-one. General plus-ones (friends, colleagues) are not permitted. Max 2 seats per RSVP.          |
| 13  | Spouse name required                   | If a guest indicates they are bringing their spouse, they must enter the spouse's full name in the RSVP form. This name appears in the admin dashboard.          |
| 14  | Home page scope                        | The home page is complete as built. No further sections are required.                                                                                            |
